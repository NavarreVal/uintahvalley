...
The hero is one lockup: stacked Quattrocento lines **PURE** and **PREMIUM**, sized with `clamp()` / `vw` so each word spans the hero width. `assets/hero-product-plate.png` (orchid, beans, and bottles) sits centered on top of the type. The shop button stays under that composition. On load, PURE pops in, then PREMIUM, then the plate flies in from the right (CSS only, for every visitor).
